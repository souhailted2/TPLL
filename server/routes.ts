...
export async function registerRoutes(
  ...
  app.post("/api/machines/seed", requireFactoryMonitor, async (req, res) => {
    try {
      ...
      for (const m of allMachines) {
        await storage.createMachine({ code: m.code, name: m.name, section: m.section, posX: null, posY: null, width: null, height: null });
      }

      res.json({ success: true, count: allMachines.length });
    } catch (err) {
      console.error("Seed machines error:", err);
      res.status(500).json({ message: "Failed to seed machines" });
    }
  });
